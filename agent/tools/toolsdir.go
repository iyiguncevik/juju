...
func UnpackTools(dataDir string, tools *coretools.Tools, r io.Reader) (err error) {
	...
	if err = createJujudCompatSymlink(dir); err != nil {
		return err
	}
...
// createJujudCompatSymlink creates a "jujud" symlink pointing at
// "jujuagentd" inside the unpacked tools directory. This ensures that
// init scripts generated by older versions of Juju (which reference the
// "jujud" binary) continue to work after the agent binary was renamed to
// "jujuagentd". The symlink is only created if the new binary exists and
// the old one does not, so archives that still ship "jujud" directly are
// left untouched.
func createJujudCompatSymlink(dir string) error {
	suffix := ""
	if runtime.GOOS == "windows" {
		suffix = ".exe"
	}
	newPath := filepath.Join(dir, names.JujuAgentd+suffix)
	if _, err := os.Stat(newPath); os.IsNotExist(err) {
		return nil
	} else if err != nil {
		return errors.Annotatef(err, "cannot stat %q", newPath)
	}
	oldPath := filepath.Join(dir, names.JujuController+suffix)
	if _, err := os.Lstat(oldPath); err == nil {
		return nil
	}
	if err := symlink.New(names.JujuAgentd+suffix, oldPath); err != nil && !os.IsExist(err) {
		return errors.Annotatef(err, "cannot create %s compatibility symlink", names.JujuController+suffix)
	}
	return nil
}
